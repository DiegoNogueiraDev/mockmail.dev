...
# Script de Teste Completo - MockMail API
# Cria usuários de teste e testa validação de emails

API_URL="http://localhost:3000"
TEST_LOG="/tmp/mockmail_complete_test.log"
PASSED=0
FAILED=0

echo "🧪 INICIANDO TESTES COMPLETOS DE EMAIL" > $TEST_LOG
...
test_email() {
    ...
    if [ "$status_code" = "$expected_status" ]; then
        ...
        echo "   Resposta: $(echo $body | cut -c1-200)..." | tee -a $TEST_LOG
        ((FAILED++))
    fi
